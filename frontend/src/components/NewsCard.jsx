...
function NewsCard({ article, index, visible }) {
  ...
  // useRef is the correct guard here — avoids stale closure bug where
  // the useEffect reads an outdated `loadingSlides` state value.
  ...
  const [saved, setSaved] = useState(() => {
    const existing = JSON.parse(localStorage.getItem("savedArticles") || "[]");
    return existing.some(a => a.id === (article.id || index.toString()));
  });
  ...
  const [chatMessages, setChatMessages] = useState([
    { type: "ai", text: "Hey! I've read the article. Ask me anything." }
  ]);

  const currentSlides = slides?.[level] || [];

  // Auto-generate slides when the card becomes visible — guarded by ref
  useEffect(() => {
    if (visible && !slides && !loadingRef.current) {
      generateSlides();
    }
  ...
  function toggleSave() {
    const key = "savedArticles";
    const id  = article.id || index.toString();
    let savedArticles = JSON.parse(localStorage.getItem(key) || "[]");
  ...
  async function sendArticleChat() {
    ...
    try {
      ...
      const data = await response.json();
      console.log("🤖 Article Bot Source:", data.source);

      setChatMessages(prev => [...prev, { type: "ai", text: data.reply }]);

    } catch {
      setChatMessages(prev => [
        ...prev,
        { type: "ai", text: "AI unavailable." }
      ]);
    }
  }
    ...
    <div className="news-card">
      ...
      <div className="card-content">
        ...
        <div className="cardContent">
          ...
          <h2 className="card-title">
            {currentSlides[currentSlide]?.title || article.title}
          </h2>

          {loadingSlides ? (
            <p className="card-desc" style={{ opacity: 0.5 }}>
              Generating summary…
            </p>
          ) : (
            <p className="card-desc">
              {currentSlides[currentSlide]?.desc || article.desc}
            </p>
          )}
        ...
        <div className="card-actions">
          ...
          <div className="left-actions">
            ...
            <div
              className={`action like-action ${liked ? "liked" : ""}`}
              onClick={toggleLike}
            >
              <img
                src={liked ? "/images/like-filled.png" : "/images/like.png"}
                className="like-icon"
                alt=""
              />
            ...
            <div
              className="action"
              onClick={() =>
                navigator.share?.({ title: article.title, text: article.desc })
              }
            >
          ...
          <button
            className={`chatbot-btn ${showBot ? "active" : ""}`}
            ...
            <div className="comments-list">
              {comments.map((comment, i) => (
                <div className="comment-item" key={i}>
                  ...
                  <div className="comment-content">
                    <div className="comment-header"><strong>You</strong></div>
            ...
            <div className="bot-body">
              ...
              <div className="bot-input">
                <input
                  ...
                  placeholder="Ask about this article..."
                />
                <button onClick={sendArticleChat}>
                  <img src="/images/send.png" alt="" />
                </button>
              </div>
